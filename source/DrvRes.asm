...
DrvNextPtr	dd	-1
DrvFlags	dw	8980h	; CHR, OPN, LV3
		dw	offset Strategy
		dw	0
DrvName		db	'UNRSLVD$'
		db	8 dup (0)
DrvCapbl	dd	1	; Capability bit. Shutdown
...
loc_1:
	...
	test	[drvflags],mask df_opbnd	; try open adapter?
	jz	short loc_2
	mov	ds,dx
	call	grOpen				; discard return code
loc_2:
	mov	ax,SUCCESS
loc_inv:
loc_ic:
...
_SetupWoL	proc	near
	...
	mov	gs,[MEMSel]
	test	[drvflags],mask df_wol
	jz	short loc_2

	mov	ax,word ptr [MacStatus.sstMACstatus]
	and	ax,(mask msbound) or (mask msopen)
	cmp	ax,(mask msbound) or (mask msopen)	; bind & open
	jnz	short loc_1
	call	grClose
loc_1:
	call	_hwSetupWoL
